...
pane.addBinding(options, "gravitationalForce", { min: -5, max: 5, step: 0.1 });
...
onBeforeRender(() => {
  for (let i = 0; i < movers.length; i++) {
    for (let j = 0; j < movers.length; j++) {
      ...
      movers[i].update();
      movers[i].checkEdges();
      const mesh = movers[i].getMesh();
      if (mesh) {
        mesh.position.x = movers[i].worldPosition.x;
        mesh.position.y = movers[i].worldPosition.y;
      }
    }
  }
});
