...
const computeEffectBackFaces = () => {
  ...
  groupRef.value?.children?.forEach((child) => {
    ...
    const distance = camera.value?.position ? _position.distanceTo(camera.value.position) : Infinity
  ...
  const distances = meshes.map((mesh) => mesh.distance).filter((d) => Number.isFinite(d))
...
<template>
  <TresGroup ref="groupRef" v-if="isLoading === false">
    <Billboard v-for="(item, i) in items" :key="item.title" :position="[...initPositions[i]]">
