...
onBeforeRender(({ elapsed }) => {
  ...
  if (cloudRef.value) {
    cloudRef.value.instance.rotation.y = elapsed * (options.speed + 0.025);
  }
  if (planeRef.value && markersRef.value) {
    planeRef.value.instance.rotation.y = xDrag.value;
...
<template>
  ...
  <Sphere ref="planeRef" :args="[1, 60, 60]" :position="[0, 0, 0]">
    <TresMeshStandardMaterial v-if="!isLoading" :map="textures[0]" :normalMap="textures[1]" />
  </Sphere>
  <Sphere ref="cloudRef" :args="[1.01, 30, 30]" :position="[0, 0, 0]">
    <TresMeshStandardMaterial v-if="!isLoading" :transparent="true" :alphaMap="textures[2]" />
  </Sphere>
  <!-- MARKERS -->
