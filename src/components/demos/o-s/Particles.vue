// Particle System Simulation // The Nature of Code // The Coding Train / Daniel Shiffman //
...
onMounted(() => {
  ...
  rf.addBinding(repellerOpts, 'posX', { min: -960, max: 960, step: 1, label: 'X' }).on(
    'change',
    ({ value }) => {
  ...
  rf.addBinding(repellerOpts, 'posY', { min: -540, max: 540, step: 1, label: 'Y' }).on(
    'change',
    ({ value }) => {
