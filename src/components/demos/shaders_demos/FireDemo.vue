...
pane.addBinding(options, 'magnitude', { min: 0, max: 5, step: 0.1 }).on('change', (ev) => {
  fireShader.uniforms.magnitude.value = ev.value;
});

pane.addBinding(options, 'lacunarity', { min: 1, max: 5, step: 0.1 }).on('change', (ev) => {
...
pane.addBinding(options, 'noiseScaleX', { min: 0.1, max: 5, step: 0.1 }).on('change', (ev) => {
  fireShader.uniforms.noiseScale.value.x = ev.value;
});
pane.addBinding(options, 'noiseScaleY', { min: 0.1, max: 5, step: 0.1 }).on('change', (ev) => {
  fireShader.uniforms.noiseScale.value.y = ev.value;
});
pane.addBinding(options, 'noiseScaleZ', { min: 0.1, max: 5, step: 0.1 }).on('change', (ev) => {
  fireShader.uniforms.noiseScale.value.z = ev.value;
});
pane.addBinding(options, 'noiseScaleW', { min: 0.1, max: 5, step: 0.1 }).on('change', (ev) => {
...
const fireShader = {
  ...
  uniforms: {
    ...
    scale: { type: 'v3', value: new Vector3(options.scaleX, options.scaleY, options.scaleZ) },
    noiseScale: { type: 'v4', value: new Vector4(options.noiseScaleX, options.noiseScaleY, options.noiseScaleZ, options.noiseScaleW) },
