...
function step() {
  ...
  for (let i = 0; i < branches.length; i++) {
    ...
    if (b.r <= ONE) { toRemove.push(i); continue }

    // 4b. Die if the tip has left the canvas
    if (b.x < 0 || b.x > 1 || b.y < 0 || b.y > 1) { toRemove.push(i); continue }
    ...
    if (branches.length + toAdd.length < MAX_BRANCHES) {
      ...
      if (Math.random() < prob) {
        ...
        if (childR > ONE) {
          toAdd.push({ x: b.x, y: b.y, r: childR, a: b.a + forkA,
                       g: b.g + 1, prevX: b.x, prevY: b.y })
        }
      }
    }
  }
...
onMounted(() => {
  ...
  simFolder.addBinding(params, 'initRadius',     { min: 0.005, max: 0.06,        step: 0.001,   label: 'Trunk radius' })
  simFolder.addBinding(params, 'branchDiminish', { min: 5e-6,  max: 0.0002,      step: 1e-6,    label: 'Diminish rate' })

  const branchFolder = pane.addFolder({ title: 'Branching' })
  branchFolder.addBinding(params, 'splitDiminish',   { min: 0.4, max: 0.95, step: 0.01, label: 'Split size ratio' })
  branchFolder.addBinding(params, 'splitAngle',      { min: 0.05, max: Math.PI, step: 0.01, label: 'Split angle (rad)' })
  branchFolder.addBinding(params, 'splitProbScale',  { min: 0.01, max: 1.0,  step: 0.01, label: 'Split probability' })

  const curveFolder = pane.addFolder({ title: 'Curvature' })
  curveFolder.addBinding(params, 'angleDrift', { min: 0.0005, max: 0.015, step: 0.0005, label: 'Angle drift' })
  curveFolder.addBinding(params, 'angleExp',   { min: 0.5,   max: 5.0,   step: 0.1,    label: 'Drift exponent' })

  const renderFolder = pane.addFolder({ title: 'Render' })
  renderFolder.addBinding(params, 'lineWidth',   { min: 0.1, max: 3.0, step: 0.05, label: 'Line width' })
