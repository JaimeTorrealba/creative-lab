...
function step() {
  ...
  for (let iter = 0; iter < params.speed; iter++) {
    ...
    const neighbours = getNeighbourIndices(childX, childY)
      .filter(a => a !== k && a !== nodeParent[k])
    let noOverlap = true
    for (let n = 0; n < neighbours.length; n++) {
      ...
      if (dist * 2 <= nodeRadius[ni] + childRadius) { noOverlap = false; break }
    }
