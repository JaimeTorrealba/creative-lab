...
watch(
  [polygonOptions, extrudeSettings],
  () => {
    generatePolygonGeo()
  },
  { deep: true }
...
const generatePolygonGeo = () => {
  ...
  const { threeShape: polygonShape } = usePolygon({
    sides: polygonOptions.sides,
...
watch(
  [ellipseOptions, extrudeSettings],
  () => {
    generateEllipseGeo()
  },
  { deep: true }
...
const generateEllipseGeo = () => {
  const { threeCurvePath: ellipseShape } = useEllipse({
    xRadius: ellipseOptions.xRadius,
    yRadius: ellipseOptions.yRadius
  })
...
<template>
  ...
  <TresMesh name="Polygon" :position-y="3" :geometry="polygonGeometry">
    <TresMeshBasicMaterial
      :side="DoubleSide"
      :color="0x00ff"
      :wireframe="polygonOptions.wireframe"
    />
  </TresMesh>
