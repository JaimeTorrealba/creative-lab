...
function connectionSubsets(connections: number, number: number) {
  ...
  return subsets(shuffle(combinations([...Array(connections).keys()]))).filter(
    (s) => s.length === number
  )
}
...
function generateLogos() {
  ...
  for (let i = 0; i < limit; i++) {
    logos.push([leftList[i % leftList.length], rightList[i % rightList.length]])
  }
}
...
function draw() {
  ...
  for (let i = 0; i < 6; i++) {
    for (let j = 0; j < 6; j++) {
      ...
      let ys = rawYs.map((v) => ((v - rawYs[0]) / (rawYs[rawYs.length - 1] - rawYs[0])) * 100 - 50)
...
onMounted(() => {
  if (canvas.value) {
    ...
    pane
      .addBinding(leftConnections, 'value', {
        min: 1,
    ...
    pane
      .addBinding(rightConnections, 'value', {
        min: 1,
        max: connections.value,
        step: 1,
